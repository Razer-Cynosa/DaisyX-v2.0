...
async def get_start_func(message, strings, edit=False):
    ...
    buttons.add(
        InlineKeyboardButton(strings["btn_channel"], url="https://t.me/RoronoaZoro_Xbot"),
        InlineKeyboardButton("👥 Support Grup", url="https://t.me/caritemanataudoi"),
    )
    buttons.add(
        InlineKeyboardButton(
            "☠ Add ZoroX to your group",
            url=f"https://telegram.me/zoroxbot?startgroup=true",
        )
    )
...
async def help_cmd_g(message, strings):
    ...
    button = InlineKeyboardMarkup().add(
        InlineKeyboardButton(text=text, url="https://t.me/BotZoroXbot?start")
    )
    await message.reply(strings["help_header"], reply_markup=button)
